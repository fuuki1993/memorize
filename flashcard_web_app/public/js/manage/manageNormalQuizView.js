class ManageNormalQuizView {
    ...
    async deleteCategory() {
        ...
        if (selectedCategory) {
            if (confirm('このカテゴリーを削除してもよろしいですか？関連する全ての問題も削除されます。')) {
                try {
    ...
    updateQuestion() {
        if (this.editingQuizId) {
            ...
            if (category && question && answer) {
                if (this.dataHandler.updateQuiz(this.editingQuizId, category, question, answer, this.uploadedImage)) {
                    this.updateQuizList();
                    this.clearInputs();
                    alert('問題が更新されました。');
                } else {
    ...
    addNewQuestion(quizId) {
        const quiz = this.dataHandler.getQuizById(quizId);
        if (quiz && quiz.type === 'normal') {
            document.getElementById('questionInput').value = '';
            document.getElementById('answerInput').value = '';
            document.getElementById('categorySelect').value = quiz.category;
